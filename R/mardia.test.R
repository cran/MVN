...
function(data,cov=TRUE,plot=TRUE){ 
     ...
     results <- list(statistic=c(g1p=g1p,skew = skew,p.value.skew = p.skew,
                small.skew = small.skew,p.value.small = p.small,
                g2p=g2p,kurtosis = kurt, p.value.kurt = p.kurt),
                method = "Mardia's Multivariate Normality Test",data.name = dname) 
     class(results) <- "htest"
     return(results)
}

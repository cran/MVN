...
\author{Selcuk Korkmaz$^1$, Dincer Goksuluk\\[0.35cm]
...
\date{\texttt{MVN} version \Sexpr{packageDescription("MVN")$Version}  (Last revision 2014-03-14)}
...
Here, \texttt{g1p}: Mardia's estimation of multivariate skew, \texttt{skew}: Mardia's skew statistic, \texttt{p.value.skew}: p-value of skew statistic, \texttt{g2p}: Mardia's g2p estimate of multivariate kurtosis, \texttt{kurtosis}: Mardia's multivariate kurtosis statistic, \texttt{p.value.kurt}: p-value of kurtosis statistic, \texttt{small.skew}: Mardia's small sample skew statistic and \texttt{p.value.small}: p-value of small sample skew statistic.
...
Another alternative is to use 2-dimensional contour graphs. We can use the \texttt{mvnPlot} function in the \texttt{MVN} package to create a contour plot for a binary data set.  Contour graphs are very useful since it gives information about normality and correlation at the same time. From contour graph below, we can say that there is a positive correlation among \emph{sepal} measures of flowers since contour lines lie around main diagonal.

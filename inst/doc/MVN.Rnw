...
\date{\texttt{MVN} version \Sexpr{packageDescription("MVN")$Version}  (Last revision 2014-11-14)}
...
\begin{enumerate}
\item[1] Compute robust Mahalanobis distances (MD($\mathrm{x}_{i}$))
\item[2] Compute the 97.5 percent-Quantile Q of the Chi-Square distribution
\item[3] Declare MD($\mathrm{x}_{i}$) $>$ Q as possible outlier
\end{enumerate}
 

For this task \texttt{mvOutlier} function can be used as follows: 
...
\item[3] Declare MD($\mathrm{x}_{i}$) $>$ AQ as possible outlier
\end{enumerate}

Likewise, \texttt{mvOutlier} function can be used as follows: 
